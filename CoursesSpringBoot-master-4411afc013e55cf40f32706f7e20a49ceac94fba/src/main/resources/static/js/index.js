    ...
    if(data.success){
      ...
      for (let i in data.data) {
              ...
              for (let y of data.data[i].ordinaryCourse) {
                  // console.log(i)
                  $('.subNavBox').find(`.ul-${i}`).prepend(`<li><a href="/course?id=${y.course.id}">${y.course.name}</a></li>`)
              }
              for (let x of data.data[i].keyCourses) {
                  // console.log(i)
                  $('.subNavBox').find(`.ul-${i}`).prepend(`<li><a href="/course?id=${x.course.id}">${x.course.name}</a></li>`)
              }
            }
      ...
      for(let i in data.data){
          ...
          <div class="group">
            <div class="g-top">
              <h3>
                ...
                <a href="/morelessons?id=${data.data[i].courseGroup.id}" class="btn btn-default btn-sm" >更多课程</a>
              </h3>
            </div>
            <div class="div-${i}"></div>
          </div>
            ...
            if(picture_url == null){
                ...
                url = picture_url
                  ...
                  $('.r-main').find(`.div-${i}`).append(`
                      ...
                      <div class="course">
                        <div class="img">
                          <a href="/course?id=${x.course.id}" title=""><img width="100%" height="100%" src="${url}"></a>
                          
                        </div>
                        <div class="course-info">
                          <a href="/course?id=${x.course.id}" title="">${x.course.name}</a>
                          <div>
        ...
        for (let x of data.data[i].ordinaryCourse) {
                  ...
                  $('.r-main').find(`.div-${i}`).append(`
                      ...
                      <div class="course">
                        <div class="img">
                          <img width="100%" height="100%" src="${x.course.picture}">
                        </div>
                        <div class="course-info">
                          <a href="course.html?id=${x.course.id}" title="">${x.course.name}</a>
                          <div>
                            <a href="javascript:void(0);" title="">
                              <i class="glyphicon glyphicon-user"></i>&nbsp;${x.course.type}
                            </a>
